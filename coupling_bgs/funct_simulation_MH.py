import coupling_bgs.funct_simulation as funct_simulation
import numpy as np
import scipy.stats
...
class Data_lapl:
    def __init__(self):
        ...
        self.N: int = None
        self.mu:float = None
        self.ii: np.ndarray = None
        self.iss: np.ndarray = None
        self.mean_y: np.float64 = None
        self.mean_y_s:np.ndarray = None
        self.N_s: np.ndarray = None
        self.N_sl : list= None 
        self.tau_k: np.ndarray = None
        self.b = None
...
class iter_value_laplace:  
    ...
    def __init__(self, data,T,a, mu_pr = 0, tau_pr = 0, rand = True,  aux_to_save = 1,b=1):
        ...
        self.a_means: np.ndarray = np.array([self.a[k]@data.N_s[k]/data.N for k in range(data.K)])
        ...
        for j in range(aux_to_save):
            ...
            i = np.random.randint(0,data.iss[k])
...
def flat_to_cartesian(flat_index, iss):
    """
    Convert a flat_index in range(np.prod(iss)) to Cartesian indices.
    
    Args:
        flat_index: single integer from range(np.prod(iss))
        iss: array of dimension sizes
    
    Returns:
        tuple of indices, one for each dimension
    """
    indices = []
    remaining = flat_index
    
    for dim_size in reversed(iss):
...
def _asymptotic_regimes_lapl(N, iss,b=1,mu=0, tau_k = None, return_a = False):
    x =Data_lapl()
    data = pd.DataFrame()
    K = len(iss)
    if tau_k is None:
        tau_k = np.ones(K)
    a = [np.random.normal(0,1/np.sqrt(tau_k[k]),size=iss[k]) for k in range(K)]
    y = np.zeros(N)
        
    xx = random.sample(range(np.prod(iss)),N) 
    ii = np.array([flat_to_cartesian(xx[n], iss) for n in range(N)])
...
def asymptotic_regimes_lapl(num, K, I=10, **kwargs):
    if num ==1:
        # dense regime
        p = 0.1/(I**(K-2))
    elif num == 2:
    ...
    iss = [I]*K
    return _asymptotic_regimes_lapl(N, iss, **kwargs)